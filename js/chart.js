...
function processCSVData(csvData, selectedYear = '2015') {
	...
	const yearFilteredData = csvData.filter(row => row.TIME_PERIOD === selectedYear);
...
// Add slider initialization function
...
function updateVisualization(selectedYear) {
	loadCsvFile(selectedYear).then(processedData => {
		data = processedData; // Update global data
		clearChart();
		createSortControls();
		renderBarChart();
	});
...
function loadCsvFile(selectedYear = '2015') {
	return d3.csv("data/cleaned/New_OECD_RenewableEnergy.csv").then(csvData => {
		return processCSVData(csvData, selectedYear);
	}).catch(error => {
...
function initialiseVisualisation() {
	...
	loadCsvFile('2015').then(processedData => {
		data = processedData;
		renderBarChart();
	});
}
...
function createLineChart() {
	...
	d3.csv("data/cleaned/New_OECD_RenewableEnergy.csv").then(csvData => {
...
function renderLineChart(data) {
	const margin = { top: 80, right: 180, bottom: 20, left: 60 };
	const width = 1000 - margin.left - margin.right;
	const height = 600 - margin.top - margin.bottom;
